...
FORCE_CLANG = (sys.argv[3] if len(sys.argv) > 3 else os.environ.get('FORCE_CLANG')) == '1'

CONFIGURATION = sys.argv[4] if len(sys.argv) > 4 else os.environ.get('CONFIGURATION', '')
CONFIGURATIONS = [CONFIGURATION] if CONFIGURATION else ['Debug', 'Release']

XP_TOOLSET = (sys.argv[5] if len(sys.argv) > 5 else os.environ.get('XP')) == '1'
